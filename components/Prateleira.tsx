...
export function Prateleira({
  ...
  const lista = (
    <ul className="faixa-scroll sangra mt-6 flex gap-3 overflow-x-auto pb-2 sm:gap-4 lg:mx-0 lg:grid lg:grid-cols-4 lg:gap-5 xl:grid-cols-5 lg:overflow-visible lg:px-0">
      {amostra.map((p, i) => (
...
function CardVerMais({ href, restantes, total, nome, feminino }: { href: string; restantes: number; total: number; nome: string; feminino: boolean }) {
  return (
    <Link href={href} className="agua bloco group flex h-full flex-col justify-between p-5 sm:p-6">
      ...
      <span className="btn btn--placa btn--placa-fio w-full transition-colors group-hover:bg-branco group-hover:text-agua">
