...
export function CompraProduto({ produto, whatsapp, base }: { produto: Produto; whatsapp: string; base: string; atendimento?: boolean }) {
  ...
  /* o body splash não tem tamanho: tem fragrância, que vem em `cores` */
  const pedeTamanho = produto.categoria_slug !== "body-splash";
  ...
  return (
    <div className="space-y-6">
      ...
      {produto.cores.length > 1 ? (
        <fieldset>
          <legend className="etiqueta mb-2">{produto.categoria_slug === "body-splash" ? "Fragrância" : "Cor"}</legend>
          <div className="flex flex-wrap gap-2">
            {produto.cores.map((c) => (
      ...
      <div className="grid gap-5 sm:grid-cols-[auto_minmax(0,1fr)] sm:items-end">
        ...
        {pedeTamanho && !produto.tamanhos.length ? (
          <label className="campo-flutuante">
            <span>Tamanho</span>
            <input value={tamanhoLivre} onChange={(e) => setTamanhoLivre(e.target.value)} placeholder="Ex.: P, M, G, ou o que você veste" maxLength={20} />
          </label>
        ) : null}
      </div>
      ...
      <div className="rounded-[var(--raio)] border border-linha p-5 sm:p-6">
        ...
        <p className="miudo mt-3">A mensagem já vai com a peça, a quantidade e o que você escolheu. A loja confirma se tem, passa o valor e combina a entrega.</p>
      </div>
    </div>
  );
}
