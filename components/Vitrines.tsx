          ...
          return (
            <li key={v.titulo}>
              <Link href={v.href} className="bloco group block">
                <span className="foto block aspect-[4/3] rounded-b-none">
                  {capa ? (
                    <Image
                      src={capa.url}
                      alt=""
                ...
                <span className="agua flex items-center justify-between gap-4 p-5 sm:p-6">
                  <span className="min-w-0">
                    <span className="manchete block text-[clamp(1.125rem,2vw,1.5rem)] text-marfim">{v.titulo}</span>
                    <span className="mt-1 block text-[0.9375rem] text-marfim-fraco">{v.texto}</span>
                  </span>
