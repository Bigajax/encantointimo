...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const descricao =
    produto.descricao ?? `${produto.nome}${produto.marca ? `, ${produto.marca}` : ""}. Na Encanto Íntimo, Itaporanga.`;
  ...
  return {
    ...
    openGraph: {
      type: "website",
      title: `${produto.nome} · Encanto Íntimo`,
      description: descricao,
...
export default async function PaginaProduto({ params }: Props) {
  ...
  const jsonLd = {
    ...
    offers: {
      ...
      ...(preco ? { price: preco } : {}),
      seller: { "@type": "Store", name: "Encanto Íntimo" },
    },
  };
      ...
      <div className="miolo pb-20 pt-6 lg:pt-10">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:gap-14">
          ...
          <div className="lg:pt-2">
            ...
            <h1 className="manchete mt-2 text-[clamp(2rem,4vw,3rem)] text-tinta">{produto.nome}</h1>
            ...
            <div className="mt-5 border-b border-linha pb-5">
              {vigente ? (
                ...
                <p className="voz text-[1.375rem] text-agua">{produto.categoria_slug === "body-splash" ? "Valor e fragrância: na mensagem." : "Valor e tamanho: na mensagem."}</p>
              )}
            </div>
        ...
        {parecidos.length ? (
          <section aria-labelledby="titulo-parecidos" className="mt-16 lg:mt-20">
            <Regua id="titulo-parecidos">
              {`Mais em ${categoria?.nome.toLowerCase() ?? "a loja"}`}
            </Regua>
